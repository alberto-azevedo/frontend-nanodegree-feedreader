...
$(function() {
    ...
    */
    ...
    /* Write a new test suite named "New Feed Selection" */
         ...
         */
			beforeEach(function(done) {
				// first change an item
				$('.feed > .entry-link').each(function (idx) {
					if (idx == 0) { // first item
						if ($( this ).text() != '') {
							$( this ).text('CHANGE TEXT'); // change item
							expect($( this ).text()).toBe('CHANGE TEXT');			
						}
					}
				});
				loadFeed(0,function() {
			...
			it('new feed is really loaded', function(done) {
				...
				$('.feed > .entry-link').each(function (idx) {
					if (idx == 0) { // first item
						if ($( this ).text() != 'CHANGE TEXT') { // reloaded?
							result = true;	
						}
					}
				});
				expect(result).toBe(true); // ensure reload
				done();
			 });
		});
}());
